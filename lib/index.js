...
global.imgMap = new Map(); // temporary solution for image
global.imgArr = [];
global.currentBase;
global.currentId;
var carbone = {
      ...
      file.openTemplate(templatePath, function (err, template) {
        ...
        preprocessor.execute(template, options, function (err, template) {
          ...
          walkFiles(template, data, options, 0, function (err, report) {
            ...
            //console.log(global.imgArr);
            report.files[3].data = report.files[3].data.replace(
              "</Relationships>",
              ""
            );
            for (let image of global.imgArr) {
              var regexId = /r:embed="rId(.*?)"/g;
              var id = regexId.exec(image.id + '"');

              const filename = id[1];

              report.files.push({
                data: Buffer.from(image.base, "base64"),
                isMarked: false,
                name: `word/media/${filename}.png`,
                parent: "",
              });

              if (image.main) {
                var checkForImage = new RegExp(
                  '<Relationship Id="rId' +
                    filename +
                ...
                report.files[3].data = report.files[3].data.replace(
                  checkForImage,
                  (match, token) => {
                    return `<Relationship Id="rId${filename}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="media/${filename}.png"/>`;
                  }
                );
              } else {
                report.files[3].data += `<Relationship Id="rId${filename}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="media/${filename}.png"/>`;
              }
            }
            report.files[3].data += "</Relationships>";
