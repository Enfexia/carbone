...
var builder = {
  ...
  assembleXmlParts: function (arrayOfStringPart, sortDepth, builderDictionary) {
    ...
    for (var i = 0; i < arrayOfStringPart.length; i++) {
      ...
      /*
      var regexImage = /data:image\/png;base64,(.*?)"/g;
      var image = regexImage.exec(_str);
      var regexId = /r:embed="rId(.*?)"/g;
      var id = regexId.exec(_str);

      if (image) {
        global.currentBase = image[0].slice(0, -1);
      }
      if (!image && _str.slice(0, 21) === "data:image/png;base64") {
        global.currentBase = _str;
      }
      if (id) {
        //console.log(id[0]);
        global.currentId = id[1];
        if (global.imgMap.has(id[1])) {
          // find and replace
          const currentID = global.imgMap.get(id[1]).num;
          if (currentID === 0) {
            global.imgArr.push({
              id: `r:embed="rId${id[1]}`,
              base: global.imgMap.get(id[1]).base,
              main: true,
            });
          }
          _str = _str.replace(id[0], `r:embed="rId${id[1]}_${currentID + 1}"`);
          global.imgMap.set(id[1], {
            num: currentID + 1,
            base: global.currentBase,
          });
          global.imgArr.push({
            id: `r:embed="rId${id[1]}_${currentID + 1}`,
            base: global.currentBase,
            main: false,
          });
        } else {
          global.imgMap.set(id[1], { num: 0, base: global.currentBase });
        }
      }
      */
      _res += _str;
    ...
    var regexImage = /<w:drawing>(.*?)<\/w:drawing>/g;
    var regexDescr = /" descr="data:image\/png;base64,(.*?)"\/>/g;
    var regexId = /r:embed="rId(.*?)"/g;
    var images = [..._res.matchAll(regexImage)];
    _res = _res.replace(regexImage, (match, token) => {
      if (regexDescr.test(token)) {
        token = token.replace(regexDescr, (match, token) => {
          global.currentBase = token;
          return `" descr="auto generated image"/>`;
        });
        token = token.replace(regexId, (match, token) => {
          global.currentId = token;
          if (global.imgMap.has(token)) {
            const currentID = global.imgMap.get(token).num;
            // find and replace
            global.imgMap.set(token, {
              num: currentID + 1,
              base: global.currentBase,
            });
            global.imgArr.push({
              id: `r:embed="rId${token}_${currentID + 1}`,
              base: global.currentBase,
              main: false,
            });
            return `r:embed="rId${token}_${currentID + 1}"`;
          } else {
            global.imgMap.set(token, { num: 0, base: global.currentBase });
            global.imgArr.push({
              id: `r:embed="rId${token}`,
              base: global.imgMap.get(token).base,
              main: true,
            });
          }
          return match;
        });
        return `<w:drawing>${token}</w:drawing>`;
      }
      return match;
    });
    return _res;
